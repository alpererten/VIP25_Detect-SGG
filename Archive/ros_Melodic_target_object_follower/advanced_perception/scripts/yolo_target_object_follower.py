...
class TargetFollower(object):  # Explicitly inherit from object for Python 2.7
    def __init__(self):
        ...
        rospy.loginfo("Target object set to: %s", self.target_object)
    ...
    def inference_callback(self, msg):
        ...
        for inference_result in msg.yolo_inference:
            if inference_result.class_name == self.target_object:
                ...
                rospy.loginfo("Target object detected: %s", inference_result)
    ...
    def move_towards_target(self, inference_result, error_x):
        ...
        rospy.loginfo('Moving towards target: error_x=%s, angular=%s, linear=%s, box_width=%s', 
                     error_x, angular_velocity, linear_velocity, inference_result.box_width)
