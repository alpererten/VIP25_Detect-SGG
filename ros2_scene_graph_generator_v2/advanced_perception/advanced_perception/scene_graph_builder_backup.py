    ...
    def print_scene_graph(self):
        ...
        for i in range(len(obj_list)):
            ...
            for j in range(i + 1, len(obj_list)):
                ...
                skip_horizontal_vertical = id1 in obj_supported_by_object or id2 in obj_supported_by_object
                same_support = obj_support_map.get(id1) == obj_support_map.get(id2)

                rel_1_to_2 = []
                rel_2_to_1 = []

                if not skip_horizontal_vertical:
                    if abs(x2 - x1) > 20:
                        if x1 < x2:
                ...
                if d1 > 0 and d2 > 0 and abs(d2 - d1) > 0.02:
                    if d1 < d2:
                        rel_1_to_2.append("in front")
                        rel_2_to_1.append("behind")
                    else:
                        rel_1_to_2.append("behind")
                        rel_2_to_1.append("in front")
